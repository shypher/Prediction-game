...
def _get_xml(path: str, params: Dict[str, Any]) -> ET.Element:
    ...
    r.raise_for_status()
    # התוכן הוא XML (לפעמים עם הצהרת encoding). נפרסר ישירות ממחרוזת.
...
def _parse_date_time(date_str: Optional[str], time_str: Optional[str]) -> Optional[dt.datetime]:
    ...
    date: 'Sep 30, 2025'
    startime: '20:45'
    נחזיר datetime נאיבי (UTC-Naive) כמו בשאר המערכת.
    """
    if not date_str:
        return None
    try:
...
def _item_to_row(item: ET.Element) -> Dict[str, Any]:
    ...
    # עונת יורוליג: מתוך gamecode (EYYYY_*)
    season = None
...
def get_schedule(season_code: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    מחזיר את כל המשחקים מ- /v1/schedules כ-rows ממופים.
    אפשר לתת seasoncode='E2025', אבל לפי הדוגמה שלך גם בלי פרמטר זה עובד.
    """
    params: Dict[str, Any] = {}
    if season_code:
        params["seasoncode"] = season_code
    root = _get_xml("/v1/schedules", params)
    # המבנה: <schedule> <item> ... </item> ... </schedule>
